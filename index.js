...
app.post('/api/validate', (req, res) => {
  ...
  // If expiry was not pre-set, start expiry from now based on type
  if (!record.expiryDate && record.expiryType) {
...
app.get('/api/admin/generate', adminAuth, (req, res) => {
  ...
  const data = loadData();
  const expiryDate = calculateExpiry(expiryType, expiryValue);
  const newKeys = [];
